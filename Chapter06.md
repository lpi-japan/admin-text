...
![「Capture」メニュー→「Interfaces」を選択します](image/Ch06/wireshark1.png){width=75%}

Webサーバーが動作しているLinux上のWebブラウザからローカル（localhost）にアクセスする場合、使用するインターフェースはLoopback:loになる点に注意してください。
...
![eth0を選択します](image/Ch06/wireshark2.png){width=75%}
...
![httpで絞り込みを行います](image/Ch06/wireshark3.png){width=75%}
